  ...
  getOptionalConfigArray: (_: string) => [
    { getOptionalString: (key: string) => getValueFromConfig(github, key) },
    {
      getOptionalString: (key: string) =>
        getValueFromConfig(enterpriseGithub, key),
    },
  ],
};
...
describe('useGitHubConfig', () => {
  it('should match github enterprise config based on the catalog entity location', () => {
    ...
    expect(matchedConfig?.getOptionalString('host')).toBe(
      enterpriseGithub.host,
    );
    expect(matchedConfig?.getOptionalString('apiBaseUrl')).toBe(
      enterpriseGithub.apiBaseUrl,
    );
    expect(matchedConfig?.getOptionalString('rawBaseUrl')).toBe(
      enterpriseGithub.rawBaseUrl,
    );
  });
