...
export type ArgoCDBuilderReturn = Promise<{
  router: express.Router;
}>;

export interface ArgoCDEnvironment {
  logger: LoggerService;
  config: Config;
}

export class ArgoCDBuilder {
  static createBuilder(env: ArgoCDEnvironment) {
    return new ArgoCDBuilder(env);
  }

  constructor(protected readonly env: ArgoCDEnvironment) {}

  public async build() {
    const logger = this.env.logger;
    const config = this.env.config;

    logger.info('Initializing ArgoCD backend');

    if (!config.has('argocd')) {
      if (process.env.NODE_ENV !== 'development') {
        throw new Error('ArgoCD configuration is missing');
      }
      logger.warn(
        'Failed to initialize ArgoCD backend: ArgoCD config is missing',
      );
      return {
        router: Router(),
      } as unknown as ArgoCDBuilderReturn;
    }
      ...
      async (request, response) => {
        ...
        return response.send(
          repoAndSource.includes(
            `${request.params.repo}/${decodeURIComponent(
