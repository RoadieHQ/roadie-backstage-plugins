...
describe('Dependabot alerts overview', () => {
    ...
    describe('GithubDependabotAlertsTable', () => {
        ...
        it('check if critical severity alerts are shown', async () => {
            ...
            expect(await rendered.findByTestId("severitiesContainer")).toBeInTheDocument();
            expect((await rendered.findAllByText('Critical severity')).length).toBeGreaterThan(0);
            expect((await rendered.findAllByText('High severity')).length).toBeGreaterThan(0);
            expect((await rendered.findAllByText('Medium severity')).length).toBeGreaterThan(0);
            expect((await rendered.findAllByText('Low severity')).length).toBeGreaterThan(0);
            expect((await rendered.findAllByTestId('severityLevel')).length).toBeGreaterThanOrEqual(4);
        });
        ...
        it('check if only low severity is shown', async () => {
            ...
            expect((await rendered.findAllByTestId('severityLevel')).length).toBeLessThanOrEqual(1);
        });
    });
});