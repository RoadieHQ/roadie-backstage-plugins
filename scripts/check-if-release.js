...
async function main() {
  ...
  const packageList = diff
    ...
    .filter(path =>
      path.match(
        /^(?:(packages|plugins\/backend|plugins\/frontend|plugins\/scaffolder-actions|plugins\/scaffolder-field-extensions|utils|plugins\/home)\/[^/]+\/package\.json|plugins\/roadie-backstage-entity-validator\/package\.json)$/,
      ),
    );
